...
<?php
    ...
    // Query to calculate average sale price for a specific store
    $querySalesRating = "SELECT AVG(Sale_Price) AS avg_salesPrice FROM sales WHERE Store_ID = $storeID";

    // Execute the query for average sale price
    $salesRating = mysqli_query($connection, $querySalesRating);

    // Check if the query for average sale price was successful
    if ($salesRating) {
        // Fetch the result
        $rowRating = mysqli_fetch_assoc($salesRating);
        $averageRating = $rowRating['avg_salesPrice'];
    ...
    // Query to calculate sales per square foot for a specific store
    $querySalesPerSqFeet = "SELECT AVG(s.Sale_Price / l.Store_Area) AS avg_salesPerSqFeet
                            FROM sales s
    ...
    if ($salesPerSqFeetResult) {
        // Fetch the result
        $row = mysqli_fetch_assoc($salesPerSqFeetResult);
        $averageSalesPerSqFeet = $row['avg_salesPerSqFeet'];

        // Display the average sales per square foot
        echo '<div class="popup-banner" style="background-color: #ffffff; padding: 10px; margin-bottom: 10px; text-align: center;">
                <span style="font-size: 2em; font-weight: bold;">Sales Per Sq Ft of Store: $ </span>
                <span style="font-size: 2em; font-weight: bold; color: #000000;">' . round($averageSalesPerSqFeet, 6) . '</span>
              </div>'; // Round to six decimal places
    } else {
        // If the query fails, handle the error
        echo "Error: " . mysqli_error($connection);
    }

    // Query to retrieve sales count for each ProductID
    $sales = [];

    $querySalesCount = "SELECT Product_ID, COUNT(*) AS SalesCount FROM sales WHERE Store_ID = $storeID GROUP BY Product_ID";
    $resultSalesCount = mysqli_query($connection, $querySalesCount);

    if ($resultSalesCount) {
        while ($row = mysqli_fetch_assoc($resultSalesCount)) {
            $sales[] = $row;
        }
    } else {
        echo "Error: " . mysqli_error($connection);
    ...
    <tbody>
        <?php foreach ($sales as $item): ?>
            <tr>
                ...
                <td><?php echo htmlspecialchars($item['SalesCount']); ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
</table>
<?php endif; ?>
        ...
        <div class="division crm-customer-service" style="color:black">
            ...
            <div id="chartContainer" style="width: 600px; height: 400px; margin: 0 auto;">
